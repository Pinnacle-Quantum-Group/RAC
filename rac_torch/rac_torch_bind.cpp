...
void rac_launch_tn(
    ...
    int M, int N, int K, float alpha, float beta,
    void* stream);
/* Tunable-precision variants (iters in [1, 24]). iters == 24 hits the
 * fast sign-XOR FMA path; iters < 24 runs a genuine N-iteration CORDIC
 * linear multiply per element, so cycle cost scales with iters. */
void rac_launch_nn_iters(
    const float* A, const float* B, float* C,
    int M, int N, int K, float alpha, float beta,
    int iters, void* stream);
...
/* Process-wide CORDIC iteration count. Set from Python via
 * `rac_cuda_ext.set_cordic_iters(n)`. Default 24 = fast path. */
...
torch::Tensor rac_matmul_forward(torch::Tensor A, torch::Tensor B, int iters) {
    ...
    rac_launch_nn_iters(
        ...
        M, N, K, 1.0f, 0.0f, iters, _get_stream());

    return C;
}

torch::Tensor rac_linear_forward(
    torch::Tensor input, torch::Tensor weight, torch::Tensor bias, int iters)
{
    ...
    rac_launch_nt_iters(
        ...
        M, out_features, in_features, 1.0f, 0.0f, iters, _get_stream());
...
std::vector<torch::Tensor> rac_matmul_backward(
    ...
    bool need_grad_A, bool need_grad_B, int iters)
{
    ...
    if (need_grad_A) {
        ...
        rac_launch_nt_iters(
            ...
            M, K, N, 1.0f, 0.0f, iters, _get_stream());
    }
    ...
    if (need_grad_B) {
        ...
        rac_launch_tn_iters(
            ...
            K, N, M, 1.0f, 0.0f, iters, _get_stream());
    }
...
std::vector<torch::Tensor> rac_linear_backward(
    ...
    torch::Tensor weight, bool need_bias_grad, int iters)
{
    int out_features = weight.size(0);
    int in_features = weight.size(1);
    ...
    rac_launch_nn_iters(
        ...
        M, in_features, out_features, 1.0f, 0.0f, iters, _get_stream());
    ...
    rac_launch_tn_iters(
        ...
        out_features, in_features, M, 1.0f, 0.0f, iters, _get_stream());
...
PYBIND11_MODULE(rac_cuda_ext, m) {
    ...
    m.def("matmul_forward", &rac_matmul_forward, "RAC matmul forward",
          py::arg("A"), py::arg("B"), py::arg("iters")=0);
    ...
    m.def("linear_backward", &rac_linear_backward, "RAC linear backward",
          ...
          py::arg("need_bias_grad"), py::arg("iters")=0);
    /* Tunable-precision knob: sets a process-wide iteration count that
     * matmul/linear use when their per-call `iters` arg is 0 (the default). */
