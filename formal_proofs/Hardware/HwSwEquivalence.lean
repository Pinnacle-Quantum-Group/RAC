...
/-- Shift quantization error: `|dequantize (m >>> i) - dequantize m · 2⁻ⁱ| ≤ q16Resolution`.
    ...
    Math: `m >>> i = m.ediv (2^i)` (Int arithmetic right shift = Int division
    by 2ⁱ).  Then
      dequantize (m >>> i) - dequantize m · 2⁻ⁱ
        = (m.ediv 2^i : ℝ) · q16Res - m · 2⁻ⁱ · q16Res
        = -(m.emod 2^i : ℝ) · 2⁻ⁱ · q16Res     [via m = ediv·2ⁱ + emod]
    Since `|m.emod 2^i| < 2^i`, the absolute value is `< q16Res`.

    PROOF (deferred): the only Mathlib v4.5.0 lemma is
    `Int.shiftRight_eq_div_pow : m >>> (n : ℤ) = m / (2^n : ℕ)` which uses
    the ℤ→ℤ shift form (n cast to ℤ).  Bridging to our `m >>> (i : ℕ)`
    requires either `HShiftRight Int Int Int` instance reasoning or a
    case-split on m (Nat / negSucc).  Self-contained but ~30 lines. -/
